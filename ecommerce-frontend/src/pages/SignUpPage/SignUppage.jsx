...
function SignUpPage() {
    ...
    const handleChangeInputPassword = (value) => {
        ...
        if (value.length > 0) {
            const elementPasswordSignIn = document.getElementById('sign-up-password-parent')
    ...
    const handleChangeConfirmInputPassword = (value) => {
        ...
        if (value.length > 0) {
            const elementPasswordSignIn = document.getElementById('sign-up-confirm-password-parent')
            elementPasswordSignIn.classList.remove('border-red-400')
            elementPasswordSignIn.classList.remove('border')
        }
    ...
    const handleSubmitForm = () => {
        if (inputGmail.length > 0 && inputPassword.length > 0 && inputConfirmPassword.length > 0) {
            mutation.mutate({
                email: inputGmail,
        ...
        if (inputPassword.length === 0) {
            const elementPasswordSignUp = document.getElementById('sign-up-password-parent')
    ...
    return (
        <div
            className="flex items-center justify-center bg-[#0000008a] h-screen"
            onClick={handleClickOutside}>
            {message?.type === 'success' && <Success mes={message.text} />}
            {message?.type === 'error' && <Error mes={message.text} />}
            <div
                className="w-[800px] h-[445px] rounded-md bg-white flex justify-between "
                ref={modalRef}>
                <div className="pt-10 px-[45px] pb-6 flex flex-col basis-8/12 ">
                    ...
                    <div
                        ...
                        <InputForm
                            ...
                            id="sign-up-password"
                            value={inputPassword}
                            onInput={(e) => handleChangeInputPassword(e.target.value)}
                        />
                        <div
                    ...
                    <div
                        ...
                        <InputForm
                            ...
                            onInput={(e) => handleChangeConfirmInputPassword(e.target.value)}
                        />
                        <div
                            className="p-2 hover:cursor-pointer"
                            onClick={() => {
                    ...
                    {data?.data?.status === 'ERR' && (
                        <span className="text-red-600">{String(data?.data?.message)}</span>
                    )}
                ...
                <div className="w-[300px] flex justify-center flex-col items-center bg-[#deebff] gap-1 basis-4/12">
                    <img src={imageLogo} alt="" className="w-[203px] h-[203px]" />
